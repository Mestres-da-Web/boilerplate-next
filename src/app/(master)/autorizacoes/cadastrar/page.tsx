...
const ProcRow = styled.div<{ $alt?: boolean }>`
  display: grid;
  grid-template-columns: 1.5fr 1fr 1fr 0.8fr 0.8fr 80px;
  border-bottom: 1px solid ${({ theme }) => theme.colors.graydf};
...
const ESPECIALIDADES = [
  { label: 'Fonoaudiologia', terapiasSem: 1, sessoesPlan: 1, valorUnit: 85 },
  ...
  { label: 'Fisioterapia', terapiasSem: 3, sessoesPlan: 3, valorUnit: 75 },
];
...
const CadastrarAutorizacaoPage = () => {
  ...
  const handleSelectPac = (id: string) => {
    setSelectedPacId(id);
    const pac = MOCK_PACIENTES.find(p => p.id === id);
  ...
  const handleRestore = (id: string) =>
    setProcs(prev =>
      prev.map(p => (p.id === id ? { ...p, removido: false } : p)),
    );
  ...
  return (
    <Page>
      {/* Breadcrumb */}
      <Breadcrumb>
        Autorizações &nbsp;›&nbsp; <span>Cadastrar</span>
      </Breadcrumb>
      ...
      <TitleRow>
        ...
        <PageTitle>Cadastrar nova autorização</PageTitle>
      </TitleRow>
      ...
      <Card>
                ...
                <ProcTable>
                  ...
                  {activeProcs.length === 0 ? (
                    <EmptyState>
                      ...
                      <EmptyText>
                        Adicione uma especialidade e suas sessões para
                        visualizar o resumo financeiro
                      </EmptyText>
                    </EmptyState>
                  ) : (
                    <>
                      {activeProcs.map((proc, idx) => (
                        <ProcRow key={proc.id} $alt={idx % 2 === 1}>
        ...
        <Footer>
          ...
          <FooterBtns>
            ...
            <BtnSave type="button" onClick={() => router.push('/autorizacoes')}>
              <IoSaveOutline />
              Salvar
            </BtnSave>
          </FooterBtns>
...
export default CadastrarAutorizacaoPage;
